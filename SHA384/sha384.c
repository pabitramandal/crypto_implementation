...
void SHA384(unsigned char *msg, uint64_t m_len, uint64_t *hash) {
	...
	sha384_padd(msg, padded_msg, m_len, pad_len);
	uint64_t size = no_block << 4;
	uint64_t M[size];
	sha384_parse(padded_msg, M, size);
	H[0] = H0; H[1] = H1; H[2] = H2; H[3] = H3; H[4] = H4; H[5] = H5; H[6] = H6; H[7] = H7;
	for(i = 0; i < no_block; i++)
		compute_sha384(&M[i << 4]);
	for(i = 0; i < 8; i++)
		hash[i]= H[i];
}

// SHA-384 Padding
void sha384_padd(unsigned char *msg, unsigned char *padded_msg, uint64_t m_len, uint8_t pad_len) {
	uint64_t i;
	for(i = 0; i < m_len; i++)
		padded_msg[i] = msg[i];
	padded_msg[i++] = 0x80;
...
// SHA-384 Parsing
void sha384_parse(unsigned char *msg, uint64_t *M, uint64_t size) {
...
void compute_sha384(uint64_t *M) {
	uint8_t t = 0;
	uint64_t W[80];
	for(t = 0; t < 16; t++)
		W[t] = M[t];
